...
if (Test-Path $pidFile) {
  ...
  if ($existingPid -match "^\d+$") {
    ...
    if ($running) {
      Write-Output "Bot ja esta ligado (PID $existingPid)."
      exit 0
    }
  }
...
$cmd = "Set-Location -LiteralPath '$projectRoot'; node start.js *>> '$logFile'"
$proc = Start-Process -FilePath "powershell" -ArgumentList "-NoProfile", "-WindowStyle", "Hidden", "-Command", $cmd -PassThru

Start-Sleep -Milliseconds 800
$running = Get-Process -Id $proc.Id -ErrorAction SilentlyContinue
if (-not $running) {
  Write-Output "Bot nao ficou em execucao. Verifique o log para a causa: $logFile"
...
Write-Output "Bot ligado com sucesso (PID $($proc.Id))."
